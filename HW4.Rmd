...
From Random Forests we see that the most valuable features are **glucose, mass, age, preganant and pedigree**.
...
# run the RFE algorithm
results = rfe(PimaIndiansDiabetes[,1:8], PimaIndiansDiabetes[,9], sizes = c(2,5,9), rfeControl = control, method = "svmRadial")
...
The top features found using RFE are **glucose, mass, age, pregnant and pedigree**.
As we can see, the two feature selection methods provide similar results.
...
set.seed(100)
sa_obj <- safs(x=train_classification[, 1:8], 
               y=train_classification[, 9],
               safsControl = sa_ctrl)

sa_obj
```

When using simulated annealing, the top 5 features were glucose, mass, insulin, pressure and pedigree. 
