...
#grow tree
tree.carseats = rpart(Sales~., data = Carseats, subset = train, method = "anova")
tree.carseats

printcp(tree.carseats) # display the results 
summary(tree.carseats) # detailed summary of splits

# plot tree 
plot(tree.carseats, uniform=TRUE, 
   main="Regression Tree")
text(tree.carseats, use.n=TRUE, all=TRUE, cex=1.5)

```

A regression tree was plotted for the Carseat Data with Sales as the response variable and all other variables as the input variables. The relevant variables used to make the tree are ShelveLoc, Price, CompPrice, Advertising and Age. The tree has 77 nodes. 
...
legend("topright", legend = c("OOB", "Test"), pch = 23, col = c("red", "blue"))
```
As seen in the plot above, for both the testign and training sets, the error reduces as the number of mtry increases.Therefore, bagging successfully reduces error. However, the training error seems to always be less than the test error. 
...
As seen in the plot above, the mean squared error reduces with more number of trees. The error reaches a minimum at about a 1000 trees. It then increases and then comes to a level.
